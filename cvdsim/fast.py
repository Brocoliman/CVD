...
def read_lut(filename: str):
    ...
    lut = torch.tensor(lut, dtype=torch.float32).cuda()
...
# Create LUT
def make_lut(lvl: float, lutdir: str = r'./cvdsim/lut'):
    # Define LUT Path
...
def change_lvl(lvl: float, lutdir: str = r'./cvdsim/lut'):
    global lut
    lut_path = os.path.join(lutdir, get_lut_fn(lvl))
    read_lut(lut_path)

def make_image(image: torch.Tensor) -> torch.Tensor: # image in PIL RGB format: Image.open(fp).convert('RGB')
    # Open Image
    image = image * 2 - 1
