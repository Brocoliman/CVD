...
def write_lut(lut: np.array) -> None:
    lut_dim = lut.shape[0]
...
#################### LUT 
# Define LUT Path
lut_path = "sim/lut/"+f"lut{lvl}".replace(".", '-')+'.txt'

# Check if LUT exists
print("Checking for LUT")
try:
    if not make_lut:
...
if make_image:  
    ...
    image = transforms.ToTensor()(image).permute(1, 2, 0) # HxWxC
    image = image * 2 - 1

    # Format dimensions HxWxC -> NxDxHxWxC
    ...
    # Save Transform: 
    transforms.ToPILImage()(res).save(save_path)
    print("Saved Image")